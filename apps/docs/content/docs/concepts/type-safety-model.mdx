...
`vs3` tracks metadata as input/output types:

- Input type: what callers provide.
- Output type: parsed/normalized value after schema validation.

If your schema transforms values, hooks and route internals use the parsed output type, not raw input.
