...
`createVerifySignatureMiddleware` checks HMAC signatures (and optional nonces) so only clients that know the secret can call your storage API. It adds `signature` to the context with the verification result and optional auth data.
...
createVerifySignatureMiddleware({
  ...
  skipPaths: ["/upload-url/health"],
  authHook: async (ctx) => {
    return { userId: ctx.metadata?.userId as string };
  },
});
```

Required headers: `x-signature`, `x-timestamp`. If `requireNonce` is true, `x-nonce` is required. Do not expose your secret to browsers; use a server endpoint to sign requests and return headers to the client.
...
You can read context from previous middlewares (e.g. `ctx.context.signature`) and add new keys. TypeScript will infer the merged context when you use the middleware in your storage's `middlewares` array.

```ts
const requireUser = createStorageMiddleware(
  { name: "require-user" },
  async (ctx) => {
    const userId = ctx.context.signature?.auth?.userId;
...
| Export | Description |
|--------|-------------|
| `createVerifySignatureMiddleware(config)` | Middleware that verifies request signature (and optional nonce), runs optional `authHook`, and adds `signature: VerificationResult` to context. |
| `createServerRequestSigner(config)` | Builds a server-side signer for adding signature headers to requests. |
| `VerificationResult` | `{ verified: true; timestamp: number; nonce?: string; auth?: { userId?: string; metadata?: Record<string, unknown> } }`. |
| `VerifySignatureMiddlewareConfig` | Extends `RequestSigningConfig` with `nonceStore?`, `authHook?`, `skipPaths?`, `onVerificationFailure?`. |
