...
Required headers: `x-signature`, `x-timestamp`. If `requireNonce` is true, `x-nonce` is required. Do not expose your secret to browsers; use the built-in `/sign-request` route to obtain headers.
